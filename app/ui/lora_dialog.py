...
メタ情報（トリガーワード等）は SHA256 を元に civitai から取得し
userdata/lora_cache/ にキャッシュされる（app/lora.py）。ハッシュ計算と
ネットワークアクセスはワーカースレッドで行い、グリッドはまずファイル名
だけで即表示 → 取得できた順にサムネが埋まる。

適用そのものはメインウィンドウが行う（apply/remove シグナル）。適用中の
状態は set_applied() で常にメイン側から供給される。

注: MiniMax H3 は CFG レスでネガティブプロンプトが存在しないため、
トリガーワードはポジティブのみを扱う（scom からの移植時に簡略化）。
"""
...
def groups_html(groups: list[str], hovered: str, is_active) -> str:
...
class LoraDialog(QDialog):
    ...
    # トリガーワードのトグル: (token, text)。token はどのリンク由来かを一意に
    # 表す識別子。メイン側は token が未挿入なら挿入、挿入済みなら削除する。
    toggle_prompt_requested = Signal(str, str)

    def __init__(self, lora_dir: Path, cache_dir: Path, parent=None):
        ...
        words_head.addWidget(QLabel("トリガーワード:"))
        words_head.addWidget(self.lbl_edited)
        words_head.addStretch(1)

        # LoRA名〜トリガーワードの情報はスクロール領域に入れ、長くても
        # 切れないようにする。ボタン類は常時見える位置に置く。
    ...
    def _visible(self, relname: str) -> bool:
        ...
        return not text or text in relname.lower()
    ...
    def _show_detail(self, relname: str) -> None:
        ...
        if meta.get("found"):
            ...
            self.lbl_base.setText(f"civitai 登録: {base}" if base else "")
    ...
    def _civitai_words(self, relname: str) -> str:
        """civitai のトリガーワードをカンマ区切り文字列で。"""
        meta = (self._meta.get(relname) or {}).get("meta") or {}
        return ", ".join(str(w) for w in (meta.get("trained_words") or []))

    def _effective_words(self, relname: str) -> str:
        """ユーザー編集があればそれを、無ければ civitai のトリガーワード。"""
        override = self._prompts.get(relname)
        if override is not None:
            return override["positive"]
        return self._civitai_words(relname)

    def _group_token(self, i: int) -> str:
        """リンク（グループ）を一意に表す識別子。メイン側の挿入区間の追跡に
        使う。ファイル名に '|' は使えないので区切りに使える。"""
        return f"{self._current_rel}|pos|{i}"

    def _refresh_words(self, relname: str) -> None:
        self._current_rel = relname
        pos = self._effective_words(relname)
        ...
        self._pos_empty = ("（情報取得中…）" if not meta
                           else "（トリガーワードなし）")
        self._hover = ""
        self._render_words()
    ...
    def _render_words(self) -> None:
        if self._pos_groups:
            self.lbl_pos.setText(groups_html(
                self._pos_groups, self._hover,
                lambda i: self._group_token(i) in self._inserted))
        else:
    ...
    def _insert(self, href: str) -> None:
        ...
        _scheme, _, idx = href.partition(":")
        try:
            i = int(idx)
            g = self._pos_groups[i]
        except (ValueError, IndexError):
            return
        self.toggle_prompt_requested.emit(self._group_token(i), g)

    def _edit_words(self) -> None:
        """トリガーワードを編集して保存する（改行=衣装グループの区切り）。"""
        ...
        lay.addWidget(QLabel(
            "トリガーワード（カンマ区切り／改行で衣装グループを分割）:"))
        ed_pos = QPlainTextEdit(_to_edit_text(pos))
        ed_pos.setMinimumHeight(120)
...
class TriggerWordsPopup(QFrame):
    """LoRA チップにマウスホバーしたとき出す、トリガーワードのポップアップ。
    LoRA ウィンドウの詳細ペインと同じ描画（groups_html）・同じトグル操作を
    行う。クリックでメインのプロンプト欄への挿入/削除をトグルする。"""

    toggle_requested = Signal(str, str)  # (token, text)
    ...
    def set_content(self, relname: str, positive: str,
    ...
    def _token(self, i: int) -> str:
        return f"{self._relname}|pos|{i}"

    def _render(self) -> None:
        html_text = groups_html(
            self._pos, self._hover,
            lambda i: self._token(i) in self._active)
    ...
    def _on_link(self, href: str) -> None:
        ...
        try:
            ...
            return
        self.toggle_requested.emit(self._token(i), g)
